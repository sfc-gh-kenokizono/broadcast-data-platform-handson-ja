...
以上はV2配布データのローカル期待値です。V2のネイティブ検証は進行中ですが、ネイティブロードは未検証（完了未確認）です。受講時は、ブロックの成功と自分の実行結果の一致を確認してください。
...
V2のNotebook実機実行は未検証です。候補環境はruntime `V2.2-CPU-PY3.11`、Python 3.11、snowflake-ml-python 1.23.0ですが、当日のパッケージ提供とGUI接続は講師と確認します。旧版で利用できたことだけでは準備完了としません。
