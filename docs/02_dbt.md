...
このファイルには、結果を読み取るだけの3つの `SELECT` があります。上から1つずつ実行し、表示された値を次の表と比べます。テーブルの作成や更新は行いません。
...
最終確認では、受講者が読みやすい全体値だけを表示します。局別の加工内容と整合性は、すでに各局のBuildで実行した15件のテストと、commonの6件のテストで確認済みです。

期待値と異なる場合やエラーが出た場合は第3章へ進まず、各局とcommonのBuild結果が `ERROR=0 SKIP=0` であることを見直します。
