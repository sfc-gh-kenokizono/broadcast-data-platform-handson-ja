...
今回の配布データでは、1,050,000件を日次集計すると1,045,496行になります。
...
- 旧小規模版と新データが混ざっていない。既存環境の場合は[第1章の注意](01_setup.md#既存の小規模版を使った方へ)に従う。
...
分展開は5局合計で約1,354万行になります。画面の待機中に同じbuildを重ねて実行しないでください。
...
2026-09-17のネイティブDBT PROJECT検証では、全5局とも4モデル・15テストに補助処理（hook）1件を含め `PASS=20` でした。
20件のテストという意味ではありません。合計の数字だけでなく、4モデル・15テストの内訳を見てください。
...
| 結果の列 | 期待値 | 確認していること |
|---|---|---|
| `RAW_ROWS` | 208,493行 | 原本の件数 |
| `CLEAN_ROWS` | 208,363行 | NW01の完全重複60行・不正区間70行を除いた件数 |
| `MART_SESSIONS` | 208,363回 | 集計後も有効な視聴回数が残っているか |
...
|---|---|---|---|
| NW01 | 208,493 | 208,363 | 207,253 |
| NW02 | 173,320 | 173,190 | 172,517 |
| NW03 | 178,353 | 178,223 | 177,612 |
| NW04 | 239,798 | 239,669 | 238,964 |
| NW05 | 250,684 | 250,555 | 249,150 |
| 合計 | 1,050,648 | 1,050,000 | 1,045,496 |
...
|---|---|---|
| NW01 | 2,688,447 | 109,919 |
| NW02 | 2,235,800 | 109,787 |
| NW03 | 2,298,124 | 109,708 |
| NW04 | 3,092,185 | 109,990 |
| NW05 | 3,221,602 | 109,910 |
| 合計 | 13,536,158 | 549,314 |
...
ネイティブDBT PROJECT検証では、全5局の成功後にcommonを実行し、hook1件を含め `PASS=9` でした。9件のテストという意味ではありません。
...
| 確認 | 期待する結果 |
|---|---|
| `COMMON.VIEWING_DAILY` の行数 | 1,045,496行 |
| 全期間・全5局のリーチ | 20,000台（同じ端末を一度だけ数える） |
| 総視聴回数 | 1,050,000回 |
| 総視聴時間 | Snowflake実測12,514,299.96658分 |
| 最初・最後の視聴日 / ジャンル数 | 2026-05-01・2026-07-31 / 8種類 |
| 局別日次マート合計との比較 | 行数差・回数差は0、分数は下記の許容誤差内 |
| `COMMON.MINUTE_AUDIENCE` の行数 | 549,314行 |
| `VIEWING_DEVICES` の総和 | 13,536,158（局・端末・分の組の延べ数） |
| 局別分単位マート合計との比較 | 行数差・端末分の差は0 |
| ラベル表と観測端末の対応 | 観測のないラベル端末・ラベル行のない観測端末とも0 |

確認SQLの途中にある `EXECUTE IMMEDIATE $$` から `$$;` までは一括実行します。
局ごとのRAW・整形後・日次・分展開・分内視聴端末数を照合し、`ACTUAL_METRICS` に結果を返します。
件数の `*_DIFFERENCE` は0、FLOATの分数差は `max(0.000001分, 比較元の分数の絶対値 × 0.000000001)` 以内を確認します。
観測された局・日ごとの分単位集計も返しますが、保存済みデータの作成・変更はしません。

新データのSnowflake/dbt実機実行で、日次1,045,496行、20,000台、1,050,000回、分単位549,314行、端末分の総和13,536,158を確認済みです。
分数のローカル計算値は12,514,299.966666667分で、Snowflake実測値との差は約0.000086667分です。各区間の固定小数点除算による丸めを含むため、完全一致とはしません。上記の許容誤差内で照合します。
**13,536,158は実視聴の分数でも全期間のリーチでもありません。** 区間端の短い視聴も1分の枠に含まれるためです。
...
- 視聴時間はナノ秒単位の差を60,000,000,000.0で割ってからFLOATへ変換します。明示的なROUNDはありませんが、FLOAT化の前に区間ごとの固定小数点除算で丸めが生じます。浮動小数点の合計にも微小誤差があるため、ローカル計算との厳密一致は求めません。
...
旧小規模版でのトライアル結果と、新データ版の実機結果は別です。
2026-09-17に新データ版の22モデル・81テストを、dbt 1.9.4 / Snowflake adapter 1.9.2のネイティブDBT PROJECTオブジェクトで確認しました。本章の `EXECUTE DBT PROJECT FROM WORKSPACE` 経路とGit Workspaceの通し操作は未確認です。
検証ログのNW01〜NW05のbuild内部時間は順に15.08 / 16.18 / 15.92 / 16.48 / 17.24秒、全5局成功後のcommonは7.96秒でした。起動待ちを含む所要時間ではなく、受講時の速度・3時間以内の完了を保証する値ではありません。
