...
補助処理の成功数をテスト数へ含めず、2モデル・6テストの内訳を確認します。2026-09-18のV2ネイティブdbt実行では、各局4モデル・15テスト、共通2モデル・6テストがすべて成功しています。`FROM WORKSPACE` 経路は別途未検証です。
...
| 総視聴回数 | 1,050,000回 |
| 総視聴時間 | 約12,511,642.266667分（Snowflake実測12,511,642.266666668分） |
| 最初・最後の視聴日 / ジャンル数 | 2026-05-01・2026-07-31 / 8種類 |
| 局別日次マート合計との比較 | 行数差・回数差は0、分数は下記の許容誤差内 |
| `COMMON.MINUTE_AUDIENCE` の行数 | 548,535行 |
| `VIEWING_DEVICES` の総和 | 13,532,927（局・端末・分の組の延べ数） |
...
上表はV2配布データのローカル集計と2026-09-18のSnowflake/dbt実行で照合済みです。総経過時間750,698,536秒から約12,511,642.266667分になります。確認SQLも例外検査を含め末尾まで成功しています。
Snowflakeでは経過ナノ秒をFLOATへ変換してから分数へ割り、合計します。浮動小数点の丸めがあるため、時間を厳密一致とはせず上記の許容誤差内で照合します。局別確認ブロックの成功に加え、共通側6テストの成功も確認してください。
...
2026-09-18にV2のネイティブDBT PROJECT（dbt 1.9.4）で22モデル・81テストの成功を確認済みです。`EXECUTE DBT PROJECT FROM WORKSPACE` とGit Workspaceからの通し操作は未検証です。
内部buildの時間は記録済みですが、起動待ちや画面操作を含む通し時間は未計測です。3時間以内の完了や性能を保証する値とはしません。
