...
このSQLファイルには、通常の `SELECT` と、`EXECUTE IMMEDIATE $$` で始まる確認ブロックが含まれます。

- 通常の `SELECT`: 件数や期間などを結果表に表示し、人が値を確認します。
- `EXECUTE IMMEDIATE $$`: 複数の照合、繰り返し、合否判定を1回で実行する **Snowflake Scripting** のブロックです。
- `$$`: ブロック全体の開始と終了を示す区切りです。入力する値ではありません。
- `DECLARE`: 照合結果を一時的に保持する変数と、不一致時のエラーを準備します。
- `BEGIN ... END`: 確認SQL、5局の繰り返し、条件判定を順に実行します。
- `RAISE`: 不一致を検出した場合にエラーで停止し、次章へ進まないようにします。

このファイルには、次の2つの確認ブロックがあります。

- 1つ目: 5局の合計と `COMMON` の日次表・毎分表を照合します。成功時は結果表を表示せず、エラーなく完了します。
- 2つ目: NW01〜NW05を順番に確認し、RAWから計算した期待値と、整形・日次・分展開・毎分集計の結果を照合します。成功時は局ごとの5行を表示します。

通常のSQLだけでも、照合ごとに別々のSQLとして実行できます。ただし、その方法では実行回数が増え、不一致の見落としも起きやすくなります。このガイドでは、**複数の確認をまとめ、不一致なら自動で停止するため**にSnowflake Scriptingを使っています。データの作成や更新は行いません。

実行するときは、`EXECUTE IMMEDIATE $$` から対応する `$$;` までを選択し、途中で区切らず一括実行してください。ブロック内の1行だけを実行すると、変数や条件分岐が成立せずエラーになります。
