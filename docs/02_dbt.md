...
補助処理の成功数をテスト数へ含めず、2モデル・6テストの内訳を確認します。V2のネイティブ実行は未検証です。
...
| 総視聴回数 | 1,050,000回 |
| 総視聴時間 | ローカル約12,511,642.266667分。SnowflakeのV2値は未検証 |
...
上表はV2配布データのローカル集計で確認した期待値です。総経過時間750,698,536秒から約12,511,642.266667分になります。Snowflake/dbtでのV2実機照合は未検証です。
Snowflakeでは各区間の固定小数点除算と合計の丸めがあるため、時間を厳密一致とはせず上記の許容誤差内で照合します。局別確認ブロックの成功に加え、共通側6テストの成功も確認してください。
...
V2のネイティブDBT PROJECT、`EXECUTE DBT PROJECT FROM WORKSPACE`、Git Workspaceからの通し操作は未検証です。旧版の成功数や所要時間をV2へ流用しません。
実行時間は未計測です。起動待ちや画面操作も含めて記録し、3時間以内の完了や性能を保証する値とはしません。
