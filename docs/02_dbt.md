...
合計の数字だけでなく、4モデル・15テストの内訳を見てください。
...
これらの期待値は配布Parquetのローカル検証値です。新データでのSnowflake/dbt実機実行は未確認です。
...
- 視聴時間はナノ秒単位の差を60,000,000,000で割ったFLOATの分数です。明示的な丸めはしませんが、浮動小数点の合計には微小な誤差が生じます。
...
本章の22モデル・81テストは現行ファイルの定義に基づきますが、新データ版のSnowflake上の実行とGit Workspace経路の通し操作は未確認です。
