...
| ビュー | 内容 | 見どころ |
|---|---|---|
| 経営サマリー | 重複除外リーチ、総視聴時間、視聴区間、1台あたり視聴、前期間比、局別構成 | 共通端末IDで重複を除いた選択局全体の姿 |
...
同じテレビ端末がNW01とNW02を見た場合、NW01のリーチでは1台、NW02のリーチでも1台です。局別リーチを単純に足すと2台になりますが、重複除外リーチでは共通の `DEVICE_ID` で名寄せして1台と数えます。

```text
同じ端末 C000123
  NW01を視聴 → NW01のリーチでは1台
  NW02を視聴 → NW02のリーチでは1台

局別リーチの単純合計       2台
重複除外リーチ             1台
```

これは人、世帯、視聴率ではありません。この教材の合成データにあるテレビ端末の重複なし件数です。全5局を選べば5局間、2局を選べばその2局間で端末の重複を除きます。
...
前期間が同じ日数だけ存在する場合だけ、4つのKPIに前期間比が表示されます。全期間を選ぶなど、同じ長さの前期間がない場合は比較表示を出しません。
