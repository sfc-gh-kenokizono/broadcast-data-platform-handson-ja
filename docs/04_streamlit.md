...
参照する `ML.PREDICTIONS` は `DEVICE_ID`、`PROB_F1`、`PREDICTED_HAS_F1`、`MODEL_NAME`、`MODEL_VERSION`、`PREDICTED_AT`、`PREDICTION_THRESHOLD`、`DATASET_VERSION` の8列です。モデルは `TV_F1_PRESENCE_MODEL`、初期版は `V3`、登録済み環境では第3章で選んだ未使用の `V4` などです。XGBoostモデルの判定境界は0.50です。日時はUTCの `TIMESTAMP_NTZ` で、1回の公開スナップショットは全20,000台が同じ `PREDICTED_AT` を持ちます。データ版は `F1_SIGNAL_V2` です。
