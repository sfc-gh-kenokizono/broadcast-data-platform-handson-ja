...
- 予測0／1は合成ラベルに対する二値分類です。確率や真の属性ではありません。選択期間は視聴端末の選別条件であり、学習期間・予測日時の条件ではありません。モデル名・バージョン・保存日時（UTC）を併記します。PREDICTED_ATは推論開始時刻ではなく結果の保存時刻です。
...
- 予測の正常性検査と表示用集計は1つのSQLで取得し、一緒にキャッシュします。古い検査結果で別時点の予測を判定しません。接続切れではゼロ件・未作成と扱わず、アプリ再起動による再接続を案内します。
