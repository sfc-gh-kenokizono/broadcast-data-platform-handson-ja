...
2026-09-17のトライアルでは、Semantic Viewと元の共通テーブルの直接SQLが、リーチ20,000台・総視聴回数1,050,000回・総視聴時間12,514,299.96658分で一致しました。
この一致はSnowflake上の2つの集計経路の比較です。ローカルの経過秒数から算出した分数との丸め差は、[第2章](02_dbt.md)の説明を参照してください。
...
2026-09-17の本編データを使ったトライアル実行結果として、次の成功報告を受けています。この文書更新ではSQLを再実行していません。

- 既存Semantic Viewをバックアップしたうえで、検証用にSQL04の作成文を `CREATE OR REPLACE` として実行し、置換に成功しました。3指標は第2節のとおり元テーブルの直接SQLと一致しました。
- 既存Agentをバックアップしたうえで、代替SQLの作成文を検証用に `CREATE OR REPLACE` として実行し、置換とアナリストへの `USAGE` 付与に成功しました。
- 参加者用SQLは引き続き新規作成の `CREATE` です。バックアップ済みの検証操作を、参加者が既存オブジェクトを置換する手順へ読み替えないでください。

GUIでの作成・保存・公開、CoWorkへの反映、Playground／CoWorkでの実際の回答と共有先利用者による動作は **UNTESTED（未検証）** です。SQL作成成功や指標一致を、回答成功・GUIとSQLの動作同等性と混同しないでください。
