...
2026-09-18にV2のSnowflake上で2経路を照合し、リーチ20,000台・総視聴回数1,050,000回・総視聴時間12,511,642.266666668分の一致を確認済みです。
時間の丸めと比較方法は[第2章](02_dbt.md)を参照してください。旧版の数値を回答の正解として使いません。
...
2026-09-18にV2データでのSemantic View・直接SQLの一致、SVのSELECT/REFERENCES、Agent代替DDLの作成とUSAGE付与を確認済みです。検証ではバックアップ後の既存オブジェクトを置換しましたが、参加者用SQLは同名オブジェクトを上書きしない新規作成用のままです。
GUIでの作成・保存・公開、CoWorkへの反映、Playground／CoWorkの回答と共有先利用者による動作も **UNTESTED（未検証）** です。文章と設定値がそろっていても、実行結果の一致まで保証されるわけではありません。
