...
| どこ | 操作 | 成功 |
|---|---|---|
| Preview | `2026年5月1日から7月31日の全5局の日別総視聴時間の推移を、折れ線グラフで表示してください`と質問 | 期間・局・単位が明示され、日別の推移が折れ線グラフで表示される |
| Preview | `2026年5月1日から7月31日の放送局別リーチを比較してください`と質問 | 局別リーチを足して全体リーチとしておらず、明示しなくても可能なら棒グラフが表示される |
...
グラフ化は応答手順による優先指示です。比較・推移の結果では可能な限り表示しますが、単一値やデータなしの回答では無理に作成しません。グラフが表示されない場合も、回答値と生成SQLの正しさを先に確認します。
